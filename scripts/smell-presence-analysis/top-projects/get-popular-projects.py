...
def get_package_list(pm):
  ...
  def parse_npm_from_npm_rank():
    file_path = os.path.join(os.path.dirname(__file__), "popular-projects/top-1000-npm.json")
    # Data comes in a list of dictionaries; from each, we want the name, and set the dependents to 0
    ...
    for pkg in data:
      name = pkg["name"]
      dependents = 0
      packages.append((name, dependents))
    return packages
